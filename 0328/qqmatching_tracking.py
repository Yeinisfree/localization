...
path = "/home/cgv/0328/part2_AF_rotated_0329/"
...
for i in range(0, num_images-1):
    ...
    kp_q, des_q = sift.detectAndCompute(img1, None)
    kp_d, des_d = sift.detectAndCompute(img2, None)
    ...
    img_matches = cv2.drawMatches(img1, kp_q, img2, kp_d, good_matches, None, flags=cv2.DRAW_MATCHES_FLAGS_DEFAULT)
    ...
    # print(f"{i+1} pose: \n", pose)
    # print(f"{i+1} rotation: \n", np.arctan2(M[1, 0], M[0, 0]) * 180 / np.pi)
    # print()
    
    cur_angle = np.arctan2(M[1, 0], M[0, 0]) * 180 / np.pi
    
    if(abs(cur_angle - prev_angle) >= 90):
        print("*** Rotation ***")
        print(f"q{i} v.s. q{i+1}")
