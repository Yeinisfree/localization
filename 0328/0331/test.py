...
if dataset == 0:
    ...
    if part == 2:
        num_images = 55
        file_dir = "/home/cgv/0328/part2_AF_0328/test"
        ratio = 0.75
    elif part == 3:
        num_images = 78
...
for i in range(1, num_images):
    ...
    src_pts, dst_pts, good_matches = track.feature_matching(images[i-1], images[i], kp1, kp2, des1, des2, "BF", ratio)
    ...
    if i == 1:
        prev_mean_direction_vector = mean_direction_vector
    cos_angle = np.dot(prev_mean_direction_vector, mean_direction_vector) / (np.linalg.norm(prev_mean_direction_vector) * np.linalg.norm(mean_direction_vector))
