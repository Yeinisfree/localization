...
class Tracking:
    ...
    #    if len(src) < min_pts or len(dst) < min_pts:
    ...
    #        return src, dst
       ...
       best_outlier_count = len(prev_pts_copy)
       best_prev_pts = prev_pts_copy
       best_next_pts = next_pts_copy
       ...
       while iteration < max_iterations:
           ...
           if len(prev_pts_copy) < min_pts and len(next_pts_copy) < min_pts:
       ...
       if iteration >= max_iterations:
           ...
           mean_direction_vector = best_mean_direction_vector
    ...
    def get_translation_Essential(self, pose, src, dst, K, section):
        ...
        E, mask = cv2.findEssentialMat(src, dst, K)
        ...
        # self.RT = self.makeRTMatrix(R, t.ravel())
        
        # self.__RT_update = self.__RT_update.dot(self.RT)
