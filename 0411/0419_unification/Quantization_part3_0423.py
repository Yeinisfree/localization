...
import time
import os
import tracking_bias_part3_0423 as tracking
...
def decision_axis(cur_db_idx, __section1, __section2, __section3, __ax):
    ...
    if(cur_db_idx <= __section1-1):
        __ax = 1
    elif(__section1 <= cur_db_idx and cur_db_idx <= __section2-2):
        __ax = 2
    elif(__section2-1 <= cur_db_idx and cur_db_idx <= __section3-1):
        __ax = 3
    else:
        __ax = 4
...
for i in range(1, num_images):
    ...
    for t, (new, old) in enumerate(zip(dst_pts, src_pts)):
        a, b = new.ravel()
        c, d = old.ravel()
    ...
    cv2.imwrite(save_path + f"p_{i-1}__c_{i}.jpg", res)
    #########
    ...
    # if(prev_rot_suspectation != 1 or rot_suspectation != 1) and __ax != 3:
