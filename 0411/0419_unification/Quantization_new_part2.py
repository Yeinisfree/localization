...
import time
import os
# import tracking_bias_new_part2 as tracking
import tracking_bias_new_part2_0423 as tracking
...
def save_DBs_pose(db_path, q_shape):
    ...
    ## section 4
    for i in range(__section3, __section4):
        __DBs_pose.append([section3_x * __scale_x, section3_y])
        section3_x -= 1
        
    section4_x = __DBs_pose[-1][0]
    section4_y = __DBs_pose[-1][1]
    
    section4_y /= __scale_y
    section4_x -= 1 * __scale_x
    
    ## section 5
    for i in range(__section4, __section5):
        __DBs_pose.append([section4_x, section4_y * __scale_y])
        section4_y += 1
        
    section5_x = __DBs_pose[-1][0]
    section5_y = __DBs_pose[-1][1]
    
    section5_x /= __scale_x
    section5_y += 1 * __scale_y
    
    ## section 6
    for i in range(__section5, __section6):
        __DBs_pose.append([section5_x * __scale_x, section5_y])
        section5_x += 1
        
    section6_x = __DBs_pose[-1][0]
    section6_y = __DBs_pose[-1][1]
    
    section6_y /= __scale_y
    section6_x += 1 * __scale_x
    
    ## section 7
    for i in range(__section6, __num_db_imgs):
        __DBs_pose.append([section6_x, section6_y * __scale_y])
        section6_y -= 1
...
def Quantization(tracking_pose, __DBs_pose, __num_db_imgs, cur_loc_db, DB_idx_list, prev_DB_idx_list, cur_db_idx, __range_x, __range_y):
    ...
    # # 전체 DB점들 중 가장 가까운 점 찾기
    # # 주변 DB가 무엇이 있는지 알아낼 수 없음
    # for i in range(0, __num_db_imgs):
    #     diff_x = tracking_pose[0] - __DBs_pose[i][0]
    #     diff_y = tracking_pose[1] - __DBs_pose[i][1]
        
    #     if(diff_x**2 + diff_y**2 < min_dist):
...
def decision_axis(cur_db_idx, __section1, __section2, __section3, __section4, __section5, __section6, __ax):
    ## __ax == 0 --> x-axis
    ## __ax == 1 --> y-axis
    ...
    if(cur_db_idx <= __section1-1):
        ...
        __ax = 3
    elif(__section3 <= cur_db_idx and cur_db_idx <= __section4-1):
        __ax = 4
    elif(__section4 <= cur_db_idx and cur_db_idx <= __section5-1):
        __ax = 5
    elif(__section5 <= cur_db_idx and cur_db_idx <= __section6-1):
        __ax = 6
    else:
        __ax = 7
...
db_path = "/home/cgv/upp/230410_part2_db/"
qpath = "/home/cgv/upp/230410_part2_q/"
save_path = "/home/cgv/0411/matching_result/newpart2/"
...
for i in range(1, num_images):
    ...
    # if(prev_rot_suspectation != 1 or rot_suspectation != 1) and __ax != 6:
    ...
    if(re == 1 or rot_suspectation == 1):
        __ax = decision_axis(cur_db_idx, __section1, __section2, __section3, __section4, __section5, __section6,__ax)
